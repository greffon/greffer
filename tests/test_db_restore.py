...
def _db_container(service="postgres", health="healthy",
                  hook="pg_restore -d app", running=True):
...
def test_db_restore_happy_leaves_running(monkeypatch):
    restart = mock.Mock()
    rdb = mock.Mock()
    cb = _restore(monkeypatch, restart=restart, rdb=rdb)
    ...
    restart.assert_called_once()                # started for pg_restore
...
def test_db_restore_data_failure_before_start(monkeypatch):
    ...
    restart = mock.Mock()
    rdb = mock.Mock()
    cb = _restore(monkeypatch, run=_run, restart=restart, rdb=rdb)
    payload = cb.call_args.args[3]
    assert payload["status"] == "failed"
    assert payload["error_code"] == "disk_full"
    restart.assert_not_called()   # never started (data overwrite failed first)
